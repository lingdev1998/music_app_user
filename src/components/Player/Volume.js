...
const Volume = (props) => {
  useEffect(() => {
    ...
    const init = () => {
      ...
      const clip = function (value, range) {
        var max = Math.max,
          min = Math.min,
        ...
        return max(
          min(range[0], range[1]),
          min(parseFloat(value), max(range[0], range[1])),
        );
      };

      // map a value from a scale to another input and output must be range arrays
      const mapToScale = function (value, rangeIn, rangeOut, reverse) {
        var max = Math.max,
          min = Math.min,
          round = Math.round,
        ...
        value =
          ((value - rangeIn[0]) / (rangeIn[1] - rangeIn[0])) *
            (rangeOut[1] - rangeOut[0]) +
          rangeOut[0];

        if (reverse)
          value = max(rangeOut[0], rangeOut[1]) + min(rangeOut[0], rangeOut[1]) - value;

        value = max(
          min(rangeOut[0], rangeOut[1]),
          min(value, max(rangeOut[0], rangeOut[1])),
        );
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    setVolume: (v) => dispatch(actions.setVolume("AudioPlaylist", v)),
  };
};
